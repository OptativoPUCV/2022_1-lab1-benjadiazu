...
void sumaNultimos(int a[], int n, int m, int * suma) {
   ...
   int sumaTotalArreglo = sumaN(a,n);
   int medio = n - m;
...
void sumaV(Vector * a, Vector * b, Vector * c) {
   ...
   {
      suma = obtenerValor(a,i) + obtenerValor(b,i);
      asignarValor(c,i,suma);
   }
}
...
void sumaV2(int a1, int a2, int b1, int b2, Vector *c){
     Vector * vectorUno = crearVector(2);
     asignarValor(vectorUno,0,a1);
     asignarValor(vectorUno,1,a2);

     Vector * vectorDos = crearVector(2);
     asignarValor(vectorDos,0,b1);
     asignarValor(vectorDos,1,b2);

     sumaV(vectorUno,vectorDos,c);
}
